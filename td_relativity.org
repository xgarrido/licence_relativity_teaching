...
1) Étant donné les valeurs des champs $\vv{E}$ et $\vv{B}$, le tenseur électromagnétique
   ...
   #+BEGIN_SRC latex
     ...
     \end{align*}
   #+END_SRC
   où $U_\nu=(\gamma, -\gamma\vv{v})=(U_0, U_1, U_2, U_3)$ et $U^\mu=(\gamma c,
...
5) [@5] On a
   ...
   #+BEGIN_SRC ipython :session :results raw drawer :exports results
     ...
     _ = plt.legend(title=r"$\uparrow\vec{E}\odot\vec{B}$", loc="upper left", bbox_to_anchor=(1,1))
   #+END_SRC

   #+RESULTS:
   :results:
   # Out[90]:
   [[file:./obipy-resources/XClz7Q.png]]
   :end:
