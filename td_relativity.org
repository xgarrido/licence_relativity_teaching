...
1) En remplaçant $\d\tau$ par $\frac{\d t}{\gamma}$ et en rappelant que
   $P^\mu=(\frac{E}{c},\vv{p})$, la composante spatialle de l'équation \ref{eq:mink} devient
   #+BEGIN_SRC latex
     \begin{align*}
...
3) [@3] Si $\vv{F}=-\vv{\nabla}V(\vv{r})=-\frac{\d V(\vv{r})}{d\vv{r}}$, on peut ainsi écrire que
   ...
   #+BEGIN_SRC latex
     \begin{align*}
       E &= \vv{p}.\vv{v}-\mathcal{L}=\gamma m\vv{v}^2+mc^2+V(\vv{r})\\
...
4) [@4]
   ...
   2) La relation de Minkowski s'écrit
      ...
      t}=\vv{E}+\vv{v}\times\vv{B}$, équation à laquelle manque la charge électrique $q$ et qui
